...
class Agent:
    """Interacts with and learns from the environment in a Sarsa manner."""
    ...
    def step(self, state, action, reward, next_state, next_action, done):
        """Processes a step and learns from the experience using one-step SARSA."""
        ...
        next_action = torch.tensor([[next_action]], dtype=torch.int64).to(device)
        done = torch.tensor([[done]], dtype=torch.float32).to(device)

        # Compute loss using SARSA target network: r + γQ(s′, a′) (if not done)
        ...
        # Check if it's time to apply the accumulated gradients
    ...
    def compute_loss(self, state, action, reward, next_state, next_action, done):
        """Computes the loss for a single experience tuple using SARSA update.
        If next_action is None (i.e. state was terminal), then the target is just reward.
        """
        with torch.no_grad():
            Q_target_next = torch.gather(
                self.Q_target(next_state), dim=1, index=next_action
            )
            Q_target = reward + (1 - done) * self.gamma * Q_target_next

        # Compute expected Q value using the local network
...
def worker_sarsa(
    ...
    """
    Function executed by each worker process using one-step SARSA.
    Args:
    ...
    for i_episode in range(1, num_episodes + 1):
        ...
        # Using local network to select first action
        action = agent.select_action(state, epsilon)

        for t in range(max_t):
            next_state, reward, done, _, info = env.step(action)

            # Using local network to select next action
...
if __name__ == "__main__":
    ...
    # Number of processes and episodes
    num_processes = 8
    num_episodes = 2000
